...
let expInfo = {'participant': '', 'Block Type': '2'};
...
function experimentInit() {
  ...
  var diffPics, encodingPics, newPics, newRetrievalPics, numEncoding, numItems, numRetrieval, pics, samePics, trialOrders, trialPics;
  ...
  random.shuffle(pics);
  trialPics = [];
  encodingPics = trialPics.slice(0, numEncoding);
  newRetrievalPics = trialPics.slice(numEncoding, (numEncoding + numRetrieval));
  newPics = [];
  trialOrders = [];
  samePics = [];
  diffPics = [];
...
function codeJSRoutineBegin(trials) {
  return function () {
    ...
    for (const thisComponent of codeJSComponents)
...
function MethodCheckRoutineBegin(trials) {
  return function () {
    ...
    MethodCheckComponents.push(mouse);
...
function Question1TextRoutineBegin(trials) {
  return function () {
    ...
    for (const thisComponent of Question1TextComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function old_new_qRoutineBegin(trials) {
  return function () {
    ...
    for (const thisComponent of old_new_qComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
